...
combined_primary <- rbind(primary_dem, primary_rep)
...
combined_primary <- combined_primary %>% clean_names() 
...
joined_data <- left_join(primary_dem, primary_rep, 
...
joined_data <- joined_data %>%
  mutate(total_votes = early_votes + election_night_votes + provisional_votes + mail_in_votes)
...
result <- joined_data %>%
  ...
  summarize(total_governor_votes = sum(governor_votes))
...
precinct_name <- max_votes_row$Precinct
voting_location <- max_votes_row$County 
...
precincts_with_zero_votes <- precinct_results %>%
  filter(total_governor_votes == 0) 
...
How many precincts are there, and which county has the most? total_precincts <- sum(precinct_counts$n) 


What else would you need to know about these precincts to know whether this might be worth a story? *Demographic Information, Historical Data, Voter Turnout, and Geographic Considerations"
...
Question: Which candidate received the highest number of votes in the Governor's race throughout all of the precincts in the dataset?

op_candidate <- result %>%
  filter(governor_votes == max(governor_votes)) %>%
  select(Precinct, Candidate) 
